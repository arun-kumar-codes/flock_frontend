  ...
  const filteredUsers = users.filter((user) => {
    ...
    if(!user.username){
     matchesSearch=user.email.toLowerCase().includes(searchTerm.toLowerCase())
      ...
      matchesSearch=user.username.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold text-slate-800">{totalUsers}</p>
            </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold text-slate-800">{creatorUsers}</p>
            </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold text-slate-800">{viewerUsers}</p>
            </div>
            <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
              <UsersIcon className="w-6 h-6 text-blue-600" />
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200">
        <div className="p-6 border-b border-slate-200">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <h3 className="text-lg md:text-xl font-semibold text-slate-800">Users</h3>
            <button
              className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 shadow-sm hover:shadow-md text-sm md:text-base"
              onClick={(e) => {
                e.stopPropagation() // Prevent click from bubbling to document
                setShowInviteModal(true)
              }}
            >
              <SendIcon className="w-3 h-3 md:w-4 md:h-4 mr-2" />
              Send Invitation
            </button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm md:text-base"
              />
            </div>
            <select
              value={filterRole}
          ...
          {isLoading ? (
            ...
            <div className="space-y-5 md:space-y-4">
              {filteredUsers.map((user) => (
                <div
                  key={user.id}
                  className="flex items-center justify-between p-1 md:p-4 border border-slate-200 rounded-lg hover:border-purple-300 hover:shadow-sm transition-all duration-200"
                >
                  <div className="flex items-center space-x-1 md:space-x-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-1">
                        <h4 className="font-semibold text-slate-800 text-xs md:text-base">{user.username}</h4>
                        <span
                          className={`px-2 py-0 md:py-1 text-xs font-medium rounded-full border ${getRoleColor(user.role)}`}
                        >
                          {user.role}
                        </span>
                      </div>
                      <p className="text-slate-600 text-xs md:text-sm mb-1 break-all block max-w-xs md:max-w-sm">{user.email}</p>
