...
const navigationItems = [
  ...
  { name: "Payout", href: "/dashboard/payout", icon: BadgeDollarSign },
];
...
function ConfirmLogoutModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative z-[101] w-[92%] max-w-sm rounded-2xl bg-white p-4 shadow-xl">
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // This “logoutRef” guard is only for the sidebar logout area,
      // the modal is outside; it closes itself via overlay/ESC.
      if (logoutRef.current && !logoutRef.current.contains(event.target as Node)) {
        // do nothing here; we don't want to auto-close the confirm flag
      }
    };
  ...
  const handleMouseEnter = () => {
    if (window.matchMedia("(min-width: 768px)").matches) setIsExpanded(true);
  };
  const handleMouseLeave = () => {
    if (window.matchMedia("(min-width: 768px)").matches) setIsExpanded(false);
  };

  const handleLogout = () => {
    // Clear tokens & state, then redirect
  ...
  const handleToggleRole = async () => {
    try {
      ...
      if (res?.status === 200) {
        ...
        // prefer dispatching your slice action creator if you have one
  ...
  // --- SAFER role check to avoid crash after logout ---
  const safeRole = (user?.role ? String(user.role) : "").toLowerCase();
  if (user?.loading) return <Loader2 />;

  // If you WANT to hard-guard creator-only access, redirect instead of throwing:
  // (This avoids calling .toLowerCase on undefined and avoids infinite Loader)
  if (!user?.id || safeRole !== "creator") {
    // You can show nothing or a tiny loader while redirecting:
  ...
  return (
    <div className={`flex h-screen bg-white ${inter.className}`}>
      ...
      <div
        className={`mobile-sidebar lg:hidden fixed inset-y-0 left-0 z-40 w-64 transform transition-transform duration-300 theme-bg-primary theme-text-primary ${
        ...
        <div className="flex flex-col bg-[#E5F6FA] h-full mx-2 my-2 rounded-4xl">
          ...
          <div className="flex items-center justify-center py-2 mt-3">
            <div className="flex items-center space-x-1">
              ...
              <span className="text-2xl font-bold text-[#2C50A2] tracking-tight transition-all duration-300">Flock</span>
            </div>
          </div>
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  >
                    <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                      <Image
                        src={user?.profile_picture || user?.profileImage || profileImg}
                        alt="User Avatar"
                        width={40}
                        height={40}
      ...
      {showLogoutConfirm && (
        <ConfirmLogoutModal onConfirm={handleLogout} onCancel={() => setShowLogoutConfirm(false)} />
      )}
    </div>
  );
}
