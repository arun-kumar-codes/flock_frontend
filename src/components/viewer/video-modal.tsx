...
export function VideoModal({
  ...
  const authorComments = video.comments.filter(
    (comment: any) =>
      comment.commented_by === video.author?.id ||
      comment.commented_by === video.creator?.id
  );
  const regularComments = video.comments.filter(
    (comment: any) =>
      comment.commented_by !== video.author?.id &&
      comment.commented_by !== video.creator?.id
  );
  ...
  useEffect(() => {
    return () => {
      ...
      console.log(
        "Component unmounting, final watch time:",
        finalWatchTime,
        "seconds"
      );
              ...
              >
                <ThumbsUpIcon
                  className={`w-5 h-5 ${isLiked ? "fill-current" : ""} ${
                    likeAnimation ? "animate-pop-purple" : ""
                  }`}
                />
                ...
                <div className="mb-6 p-4 theme-bg-secondary rounded-lg border-l-4 border-purple-500">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="font-medium theme-text-primary text-sm">
                      {video.creator?.username || video.author?.username}{" "}
                      (Creator)
                    </span>
